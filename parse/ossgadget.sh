...
SEVERITY_HIGH=$(echo $OUTCOMES | grep -o "[0-9]* Severity Critical" | awk '{print $1}')
SEVERITY_MEDIUM=$(echo $OUTCOMES | grep -o "[0-9]* Severity Important" | awk '{print $1}')
SEVERITY_LOW=$(echo $OUTCOMES | grep -o "[0-9]* Severity Moderate" | awk '{print $1}')
